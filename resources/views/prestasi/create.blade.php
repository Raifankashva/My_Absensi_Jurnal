...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="mb-6 text-sm font-medium">
        <ol class="flex items-center space-x-2">
            ...
            <li class="flex items-center">
                ...
                <span class="ml-2 text-blue-600">Tambah Prestasi</span>
            </li>
        </ol>
    </nav>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-blue-50">
            <h2 class="text-xl font-bold text-gray-900">Tambah Prestasi Baru</h2>
            <p class="mt-1 text-sm text-gray-600">Isi formulir di bawah untuk menambahkan prestasi baru untuk {{ $userSchool->nama_sekolah }}</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('prestasi.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <select name="tahun" id="tahun" 
                        ...
                        @php
                            ...
                            for($i = $currentYear; $i >= $currentYear - 10; $i--) {
                                echo '<option value="'.$i.'" '.(old('tahun') == $i ? 'selected' : '').'>'.$i.'</option>';
                            }
                        @endphp
                    </select>
                    @error('tahun')
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" 
                              ...
                              placeholder="Jelaskan detail prestasi yang diraih...">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-8 flex justify-end space-x-3">
                <a href="{{ route('prestasi.index') }}" class="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Batal
                </a>
